...
def extract_bvp(data, i, window_size, sampling_rate):
    bvp = data['BVP'][i:i+window_size].to_numpy()
    return extract_bvp_fragment(bvp, sampling_rate)
...
def extract_ecg(data, i, window_size, sampling_rate):
    ecg = data['ECG'][i:i+window_size].to_numpy()
    return extract_ecg_fragment(ecg, sampling_rate)


@features_memory.cache
def extract_eda_fragment(eda, sampling_rate):
...
def extract_eda(data, i, window_size, sampling_rate):
    eda = data['EDA'][i:i+window_size].to_numpy()
    return extract_eda_fragment(eda, sampling_rate)